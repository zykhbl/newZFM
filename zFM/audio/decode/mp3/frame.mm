...
void free_frame(struct frame **fr_ps) {
    free(*fr_ps);
    *fr_ps = NULL;
}

int seek_sync(struct bit_stream *bs, unsigned long sync, int N) {
...
void decode_info(struct bit_stream *bs, struct frame *fr_ps) {
...
void hdr_to_frps(struct frame *fr_ps) {
...
void buffer_CRC(struct bit_stream *bs, unsigned int *old_crc) {
    *old_crc = (unsigned int)getbits(bs, 16);
}

int main_data_slots(struct frame *fr_ps) {//根据帧头信息，返回当前帧音频主数据的slot个数
    int nSlots;
